...
def setup_logging(
    ...
    if json_format:
        all_handler = logging.handlers.RotatingFileHandler(
            log_path / "info.log", maxBytes=10 * 1024 * 1024, backupCount=5, encoding="utf-8"  # 10MB
        )
    ...
    error_handler = logging.handlers.RotatingFileHandler(
        log_path / "error.log", maxBytes=10 * 1024 * 1024, backupCount=5, encoding="utf-8"  # 10MB
    )
...
def log_api_call(logger: logging.Logger, api_name: str, video_id: str = None, **kwargs):
    ...
    logger.debug(f"API call: {api_name}", extra={"api_call": api_name, "video_id": video_id, **kwargs})


def log_cost(logger: logging.Logger, operation: str, cost: float, video_id: str = None):
    """Log cost information"""
    logger.info(f"Cost: {operation} = ${cost:.4f}", extra={"cost": cost, "operation": operation, "video_id": video_id})
...
def log_error(
    ...
    logger.error(
        message, extra={"error_type": error_type, "video_id": video_id, "game_name": game_name}, exc_info=exc_info
    )
